 ...
 * 初始化操作按钮
 */
const initHandleBtn = computed(() => props.handleBtn as handleType)
 ...
 * 初始化表格顶部按钮设置
 */
const initTbHeaderBtn = reactive(props.tbHeaderBtn as Btn[])
        ...
        >
          <template #default="scope">
            ...
            <template v-else-if="item.type === 'icon'">
              <svg-icon :name="scope.row[item.prop]" width="1rem" height="1rem" />
            </template>
